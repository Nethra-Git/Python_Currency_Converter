...
def currency_converter(data = None):
    ...
    if amount <= 0:
        return {"Bad Request": "Amount should be a positive number"}
    ...
    if currency_src == currency_dest:
        return {"error": "Invalid operation, Change Destination currency to SEK"}
    ...
    if currency_dest and currency_dest not in data['rates']:
        return {"error": "Change Destination currency to SEK"}   
